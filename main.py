...
UZCARD = os.getenv("UZCARD")
VISA = os.getenv("VISA")

bot = Bot(token=BOT_TOKEN)

SECOND_BOT_TOKEN ="7507583978:AAFzATDq8GIwVXDTlmENRyGugLFiabKU-NI"
...
async def premium_callback(callback: CallbackQuery):
    ...
    if callback.data.startswith("approve_"):
        ...
        cursor.execute("""
        INSERT OR REPLACE INTO english_premium
        (user_id, expire_date)
        VALUES (?, ?)
        """, (user_id, expire_date))
...
async def chat(message: Message):
    ...
    if user_id not in daily_requests:
        daily_requests[user_id] = 0
    ...
    if (
        text == "🇬🇧 English"
        and language_change_mode.get(
            message.from_user.id
        )
    ):

        language_change_mode.pop(
            message.from_user.id,
            None
        )
    ...
    if text == "🌍 Find Partner":
        ...
        print("CURRENT USER =", message.from_user.id)
    ...
    if text == "⭐ Check Status":
        ...
        if is_english_premium(message.from_user.id):
...
async def start_admin_bot():
    await admin_dp.start_polling(second_bot)
